...
홈의 `오늘의 7분 퀘스트`는 `듣고 따라 말하기 2회 → 영어 장면 고르기 2회 → 한글→영어 조립 2회 → 약한 연결 복습 1회` 순서로 자동 진행됩니다.
`힌트 사다리`는 `한글 → 영어 조립` 안의 보조 풀이로, `문장 빌드 맵`은 `사전 탐험` 안의 동사 선택 코너로 들어갑니다.

보조 코너는 따로 분리해 두었습니다.

- **힌트 사다리**: 한글 → 영어 조립 안에서 풀이 힌트로만 사용
- **문장 빌드 맵**: 사전 탐험 안에서 어떤 동사를 먼저 써야 할지 고르는 훈련

## 학습 설계

- 기본 동사(예: `get`, `make`, `take`, `have`)를 단순 번역이 아니라 **의미 엔진**으로 학습합니다.
- `동사 → 생각 틀 → 핵심 명사/장면` 구조로 표현을 연결합니다.
- 힌트는 `koen` 안의 보조 풀이로, 문장 빌드 맵은 사전 탐험 안의 코너로 제공합니다.
