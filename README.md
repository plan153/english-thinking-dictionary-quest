...
`힌트 사다리`는 `한글 → 영어 조립` 안의 보조 풀이로, `문장빌드맵`은 `동사 카드`에서 여는 콘텐츠 원본으로 들어갑니다.

보조 코너는 따로 분리해 두었습니다.

- **힌트 사다리**: 한글 → 영어 조립 안에서 풀이 힌트로만 사용
- **동사 카드**: 핵심 동사를 찾는 탐색 화면
- **문장빌드맵**: 기본 엔진 → 생각틀 → 실생활 표현을 보여 주는 원본 화면

## 학습 설계

- 기본 동사(예: `get`, `make`, `take`, `have`)를 단순 번역이 아니라 **의미 엔진**으로 학습합니다.
- `동사 → 생각 틀 → 핵심 명사/장면` 구조로 표현을 연결합니다.
- 힌트는 `koen` 안의 보조 풀이로, 문장빌드맵은 동사 카드와 같은 데이터 원본에서 제공합니다.
