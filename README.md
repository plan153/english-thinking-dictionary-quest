...
- [간극(Gap) 루프 계획서](docs/GAP_LOOP_PLAN.md) ← 자기메우기 · 힌트화 · Obsidian Gaps 구현 SoT
...
홈의 `오늘의 말`은 **순서 클리어 퀘스트가 아닙니다.**  
듣기 / 따라하기 / 이어묻기 / 이어답하기 / 이어말하기 / 화제전환 중 골라 확장하고, 연습 후에는 같은 선택판으로 돌아와 이어갑니다 (`dailyQuestV1` v6).

보조 코너는 따로 분리해 두었습니다.

- **힌트 사다리**: 한글 → 영어 조립 안에서 풀이 힌트로만 사용
- **문장빌드맵**: 동사 카드(또는 전체 맵 화면)에서 동사→생각틀→표현을 고르는 훈련
- **그래프**(메인 메뉴): Obsidian 볼트 Graph view 열기 (Local REST 필요)
...
| Active Speaking Set 출제 제한·해금 | 동작 (Starter 40 + pack_1–3 · verb have→get→take→…) |
| 구동사 메뉴 (별도 은행) | 동작 (Unlock pack≥1 · `getUnlockedBank` 비합류) |
