...
| Brain State | Obsidian `Learners/<id>/Learning/Brain State.md`에 기록되는 현재 Active set과 약점 요약 |
...
### 단일동사 커리큘럼 비중 (Starter)

| 동사 | 목표 비중 | Starter 카드(40개 기준) |
| --- | --- | --- |
| have | ~40% | 16 |
| get | ~25% | 10 |
| take | ~15% | 6 |
| 나머지(need/want/go/come/make) | ~20% | 8 |

데일리 퀘스트 후보 점수에도 `verbCurriculumWeights`를 반영한다.  
**구동사**는 [`PHRASAL_VERBS.md`](./PHRASAL_VERBS.md) · `data/phrasal-verbs.json` · 앱 `구동사` 메뉴. Unlock pack ≥1이면 열리며, 기본 퀴즈 은행에는 합류하지 않는다.
...
1. 표현 단위(**내 표현**): 연결도 `recognition`·`assembly`·`output`이 모두 강함이면 우선 인정. 아니면 성공 횟수 ≥ masteryThreshold(기본 3)이면서 **output 축 strength > 0**일 때만 인정(시도만으로는 인정하지 않음). (`isMineExpression`)
2. 팩 단위(**지금 레벨업 스토리**): Active set 내 “내 표현” 비율이 임계값(70%)을 넘으면 다음 Unlock pack(표현 약 10개)만 연다.
3. 동사 단위: **지금 해금된** 동사들의 매트릭스 4형태(평서/의문/부정/짧은 답)를 모두 통과하면 다음 `verbUnlockPacks` 한 묶음만 연다 (스타터(have·get·take·want·need·be·do·feel) → go/come/make → give → put → keep → find). 표현 Unlock pack과 별 게이트.
...
- Obsidian으로 보낼 때 `Learners/<id>/Learning/Brain State.md`와 `Next Practice.md`에 Active set과 약한 슬롯을 쓴다.

## 구현 상태

| 항목 | 상태 |
| --- | --- |
| Starter 동사 have·get·take·want·need·be·do·feel · 표현 ~106 출제 | 구현 (`getUnlockedBank`; 이후 팩 go/come/make→…) |
| Unlock pack 70% / pack_1(+10) · pack_2(+9) · pack_3(+9 keep/feel) | 구현 |
| 묻기·답하기·시제 매트릭스 | 구현 (`qa-matrices.json` + mode `matrix`, give/be/do/put/keep/find 포함) |
| Brain State / Next Practice export·import | 구현 (Local REST/Bridge + 세션 연동) |
| 동사 4형태 통과 후 새 동사 순차 해금 | 구현 (스타터(have·get·take·want·need·be·do·feel) → go/come/make → give → put → keep → find) |
| Local REST 자동 sync | 구현 (Phase 3) |

## 구현 체크리스트

- [x] Starter set ID 목록을 데이터 파일로 고정 (`data/learning-paths.json` → `activeSpeakingSet`)
- [x] 출제 게이트를 Active set으로 통일 (`getUnlockedBank`)
- [x] 해금 임계값과 Unlock pack 정의 (70% / pack_1–3)
