...
3. 동사 단위: **지금 해금된** 동사들의 매트릭스 4형태(평서/의문/부정/짧은 답)를 모두 통과하면 다음 `verbUnlockPacks` 한 묶음만 연다 (give → be …). 표현 Unlock pack과 별 게이트.
...
| Unlock pack 70% / pack_1(+10) · pack_2(+9) | 구현 |
| 묻기·답하기·시제 매트릭스 | 구현 (`qa-matrices.json` + mode `matrix`, give/be 포함) |
| Brain State / Next Practice export·import | 구현 (Local REST/Bridge + 세션 연동) |
| 동사 4형태 통과 후 새 동사 순차 해금 | 구현 (`verb_pack_give` → `verb_pack_be`) |
...
- [x] 해금 임계값과 Unlock pack 정의 (70% / pack_1 10개 · pack_2 9개)
