...
1. 표현 단위: 해당 표현의 `recognition`·`assembly`·`output` 중 **2개 이상**이 일정 강도(≥0.5)이거나, 세 연결이 모두 강하면 “내 표현”으로 본다. 성공 횟수만으로는 부족하고, 최소 1개 연결과 함께 masteryThreshold를 만족해야 한다.
2. 팩/레벨 단위: Active set 내 “내 표현” 비율이 임계값(초기 0.7)을 넘으면 **레벨 테스트**가 열린다. 테스트를 통과해야 다음 Unlock pack(표현 약 10개) / 다음 레벨이 열린다.
3. **나선(spiral):** 상위 레벨이 열려도 하위 레벨 표현은 연습 은행에 남는다. 데일리·복습은 하위 표현을 일정 비율 이상 섞는다.
4. 동사 단위: 어떤 동사의 대표 매트릭스 4형태(평서/의문/부정/짧은 답)를 통과하기 전에는 새 동사를 해금하지 않는다(후속 구현).
5. Vault overlay에서 들어온 단어는 자동 해금하지 않는다. 사용자가 “다음에 배우고 싶음”으로 표시한 뒤, Unlock pack 후보에만 넣는다.

## 레벨 테스트 (초기)

- 대상: 현재 레벨(예: L1 Starter 40)
- 개방 조건: 내 표현 비율 ≥ unlockThreshold
- 구성: 장면 고르기 · 한글→영어 · mini-matrix(평서→의문→부정) 혼합
- 통과: 세션 정답률 ≥ passScore(초기 0.7)
- 통과 기록: `curriculum.passedLevelTests`에 레벨 ID 추가 후 pack 해금

## 웹앱 반영 지점

- `getUnlockedBank()` 또는 동등 함수가 모든 출제·사전 탐험·복습의 단일 게이트가 된다. 상위 레벨에서는 하위 ID를 포함한다.
- 홈/성장에 Active set·**현재 레벨**·레벨 테스트 CTA를 보여 준다.
- Obsidian으로 보낼 때 `Learning/Brain State.md`와 `Learning/Next Practice.md`에 Active set과 약한 슬롯을 쓴다.
- 데일리 퀘스트 matrix 스텝은 mini-matrix(평서·의문·부정)로 둔다.
...
- [x] 레벨 테스트 + 나선 출제 (curriculumLevels)
- [ ] 동사 매트릭스 4형태 게이트
- [ ] ASS 표현 큐레이션 패스
