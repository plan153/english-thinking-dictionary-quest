...
3. 동사 단위: **지금 해금된** 동사들의 매트릭스 4형태(평서/의문/부정/짧은 답)를 모두 통과하면 다음 `verbUnlockPacks` 한 묶음만 연다 (have → get → take → 나머지 → give → be → do → put → keep → find). 표현 Unlock pack과 별 게이트.
...
| 동사 4형태 통과 후 새 동사 순차 해금 | 구현 (have → get → take → 나머지 → give → be → do → put → keep → find) |
