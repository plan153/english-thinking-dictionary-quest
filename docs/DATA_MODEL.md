...
첫 버전의 목표는 하나의 표현 카드를 듣기, 따라 말하기, 한영 퀴즈, 영한 퀴즈, 빈칸 퀴즈, 보조 힌트, 복습 추천에 공통으로 쓰는 것이다. 문장빌드맵은 동사 카드와 분리된 원본 화면으로 제공한다.
...
## 오늘의 퀘스트 v1 (`daily.done`)
- 홈 화면의 데일리 진행 상태는 `daily.done` 하나로만 관리한다.
- `daily.done`은 오늘의 7단계 퀘스트를 완료할 때마다 쌓이는 `quest:` 접두사 토큰 배열이다.
- `updateDashboard()`는 `quest:` 토큰 개수만 세어 `오늘의 최소 루프` 카드에 `0 / 7` ~ `7 / 7`로 표시한다.
- 일반 퀴즈, 동사 카드, 문장빌드맵 성공은 `daily.done`에 직접 들어가지 않는다.
- 날짜가 바뀌면 `daily.done`은 새로 시작한다.

### `daily.done` 구조
```json
{
  "date": "YYYY-MM-DD",
  "done": ["quest:step-1", "quest:step-2", "quest:step-3"]
}
```

### 집계 규칙
1. 오늘 날짜가 아니면 완료 개수는 0으로 본다.
2. 동일 퀘스트 스텝은 하루에 한 번만 카운트한다.
3. 동사 카드, 문장빌드맵, 일반 퀴즈 성공은 `daily.done`에 직접 반영하지 않는다.
4. 홈에는 `듣고 따라 말하기 1세트 → 영어 장면 고르기 2회 → 한글→영어 조립 2회 → 약한 연결 복습 1회`처럼 보이게 보여 주되, 실제 카운트는 7개의 퀘스트 스텝 토큰으로 유지한다.
## 확장 규칙
