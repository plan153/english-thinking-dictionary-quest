...
첫 버전의 목표는 하나의 표현 카드를 듣기, 따라 말하기, 한영 퀴즈, 영한 퀴즈, 빈칸 퀴즈, 보조 힌트, 복습 추천에 공통으로 쓰는 것이다. 문장 빌드 맵은 사전 탐험 안의 코너로 제공한다.
...
- 힌트: `hints`(단, 메인 메뉴가 아니라 한글 → 영어 조립 안의 보조 풀이)
...
## 오늘의 말하기 · 확장 선택 (`dailyQuestV1` v6 + `daily.done`)
- 홈은 **순서 클리어 퀘스트가 아니라** 확장 선택판이다: 듣기 / 따라하기 / 이어묻기 / 이어답하기 / 이어말하기 / 화제전환.
- `dailyQuestV1.expandCount`가 오늘 이어간 연습 횟수(동기 미터). 링·「이어가기 N」은 최대 7까지 표시하지만 **7에 도달해도 끝나지 않는다**.
- `daily.done`에는 확장 성공 시 `quest:expand:<choiceId>:<expressionId>` 토큰이 쌓인다(중복 방지·이력).
- 연습 후에는 강제 다음 스텝이 아니라 **같은 선택판**으로 돌아온다.
- 일반 퀴즈·사전 탐험·연결 지도 성공은 확장 토큰에 직접 들어가지 않는다(데일리 선택으로 시작할 때만).
- 날짜가 바뀌면 `dailyQuestV1`과 `daily.done`을 새로 시작한다.

### `daily.done` 구조 (v6)
```json
{
  "date": "YYYY-MM-DD",
  "done": ["quest:expand:listen:expr-1", "quest:expand:ask:expr-1"]
}
```

### 집계 규칙
1. 오늘(서울) 날짜가 아니면 이어가기 미터는 0으로 본다.
2. 표시용 미터는 `expandCount`를 7에 캡한다. 실제 연습은 계속 가능하다.
3. 동일 `choice + expression` 토큰은 `daily.done`에 하루 한 번만 쌓인다.
4. 홈 카피는 「클리어」가 아니라 「이어서 골라 확장」이다.
## 확장 규칙
