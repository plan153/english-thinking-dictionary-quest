...
- `daily.done`은 오늘 처음 성공한 학습 모드 이름들을 중복 없이 모아 둔 배열이다.
- `updateDashboard()`는 오늘의 완료 개수를 `daily.done.length / 7`로 계산해 `오늘의 최소 루프` 카드에 표시한다.
- `daily.done`에 들어가는 값은 `listen`, `speak`, `koen`, `enko`, `hint`, `build`, `review` 같은 모드 ID다.
- 같은 모드를 하루에 여러 번 성공해도 카운트는 한 번만 오른다.
...
{
  "date": "YYYY-MM-DD",
  "done": ["listen", "speak", "koen"]
}
```

### 집계 규칙
1. 오늘 날짜가 아니면 완료 개수는 0으로 본다.
2. 동일 모드는 하루에 한 번만 카운트한다.
3. 사전 탐험, 연결 지도, 일반 퀴즈에서 성공해도 해당 모드 ID만 중복 없이 집계한다.
4. 홈의 데일리 상태는 별도의 3문장 전용 저장값에 의존하지 않는다.
## 확장 규칙
