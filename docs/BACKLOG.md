...
> 기준 tip: **v1.1.7** (`91710cc`, 2026-07-23).  
...
| --- | --- | --- | --- | --- |
| F1 | **구동사 심화** | 메뉴·잠금(Unlock pack≥1)·그룹 연습(listen/speak/koen/enko)만 | (1) 그룹 순차 해금(get장소→come/go→put→keep/find/make sure) (2) 구동사 전용 매트릭스/입자 드릴 (3) 하루 루프에 구동사 스텝을 **넣지 않는** 규칙 유지 확인 | [`PHRASAL_VERBS.md`](./PHRASAL_VERBS.md) |
| F2 | **IA 2차 정리** | 내비에서 맵→구동사 교체, 성장=학습/동기화 분리, 홈 카피 조정 | 홈이 아직 “미니 대시보드” 느낌이면 Active set·모드 그리드를 더 접기/뒤로. 레슨 exit이 항상 `play`로 가는 동선 재검토(퀘스트/구동사 출처별 복귀) | 디자인 감사 잔여 |
...
2. **F1** 구동사 그룹 순차 해금 + (선택) 입자 드릴
3. **F2** IA 2차 (홈 단순화·레슨 복귀 동선)
