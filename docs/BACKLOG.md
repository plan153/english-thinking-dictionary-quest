...
> SoT: 이 문서.  
> 기준 tip: **v1.3.13** (영작 기본 · KO TTS · 정답 후 확장). 이전: v1.3.12 듣기·말하기 회귀.  
...
| G1 | **간극 자기메우기 루프** | **완료 v1.3.52** — 자동 빈 gap/Draft 중단 · 정답 선공개 제거 · 놓친 단서 필수 · 간극→힌트 · `reviewed` 메움 · Vault 경로 정합 · 계획서 [`GAP_LOOP_PLAN.md`](./GAP_LOOP_PLAN.md) |
| G2 | 듣기/말하기 실패 화면에 간극 CTA | 미착수 · Phase B |
| G3 | Next Practice 다중 간극 세션(큐 순회) | 미착수 · Phase C |
| G4 | Vault-only gap `english` 표시 보강 | 미착수 · Phase D |
...
| O5 | **퀴즈·성장 UX 복원** | **완료 v1.3.1+ / v1.3.22** — 레슨·Next Practice·복습·정답 피드백·맵/사전 상세에서 표현 연결도 UI 제거 · 음성 기본/타이핑 선택 |
...
| O12 | **이어묻기·이어답하기 Q/A** | **완료 v1.3.10** — 평서→질문 / 질문→짧은 답 문장 생성 + 듣기·말하기 4버튼 |
| O13 | **매트릭스 자동 생성** | **완료 v1.3.11** — 없는 표현은 상황 맞는 Q/A 매트릭스 생성(`qa-matrix-generate` + ASS/unlock backfill) |
| O14 | **듣기·말하기 회귀** | **완료 v1.3.12** — Safari 제스처 유지(`recognition.start` 동기) · TTS cancel 직후 재생 · 따라하기=즉시채점 |
| O15 | **영작 연습 UX** | **완료 v1.3.13** — KO 기본·KO TTS·영어로 말하기+힌트·정답 옆 EN 듣기·확장 선택·연습기록 볼트 sync |
...
1. 간극 루프(G1) 사용: 틀림 → 스스로 단서 → Gaps sync → 다시 맞춤 · 계획서 [`GAP_LOOP_PLAN.md`](./GAP_LOOP_PLAN.md)
2. 남은 간극: G2 듣기 CTA · G3 큐 순회 · G4 vault-only 표시 (Phase B–D)
3. P2 토글을 끄고 쓰려면 성장 화면 정책 체크박스
